...
describe('BookWeightingApp', () => {
    const mockGetToReadBooks = GoodreadsCSVService.getToReadBooks as jest.MockedFunction<typeof GoodreadsCSVService.getToReadBooks>;
    const mockApplyWeights = BookWeightingService.applyWeights as jest.MockedFunction<typeof BookWeightingService.applyWeights>;
    ...
    describe('run', () => {
        it('should complete the full workflow successfully', async () => {
            ...
            expect(GoogleSheetsService.writeWeightedBooksToSheet).toHaveBeenCalledWith('fake-sheet-id', mockWeightedBooks);

            // Verify console output includes key information
            expect(consoleSpy).toHaveBeenCalledWith('📚 GoodReads Book Weighting System');
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Found 2 books on your to-read shelf'));
        });
        ...
        it('should handle errors gracefully', async () => {
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith('❌ An unexpected error occurred:', 'CSV file not found');
