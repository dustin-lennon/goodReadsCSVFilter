...
describe('Integration: Complete Weighting Flow', () => {
  ...
  it('should apply 5x weight to next books in active series', async () => {
    ...
    const nextUnholyBook = weightedBooks.find((wb: WeightedBook) =>
      wb.book.Title.includes("Witch's Twilight"),
    );
    expect(nextUnholyBook?.weight).toBe(5);
    expect(nextUnholyBook?.reason).toContain('Next book');

    const nextColterBook = weightedBooks.find((wb: WeightedBook) =>
      wb.book.Title.includes('The Final Twist'),
    );
    expect(nextColterBook?.weight).toBe(5);
    expect(nextColterBook?.reason).toContain('Next book');

    // Random books should have 1x weight
    const randomBook = weightedBooks.find(
      (wb: WeightedBook) => wb.book.Title === 'Some Random Book',
    );
  ...
  it('should filter books by shelf correctly', async () => {
    ...
    const currentlyReadingBooks = await GoodreadsCSVService.getBooksByShelf(
      testCSVPath,
      ShelfType.CURRENTLY_READING,
    );
    const readingNextBooks = await GoodreadsCSVService.getBooksByShelf(
      testCSVPath,
      ShelfType.READING_NEXT,
    );
